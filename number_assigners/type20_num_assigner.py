...
class EquationSet:
    ...
    def solve_equation(self):
        x = Symbol("x")
        y = Symbol("y")
        answer = solve([self.n1 * x - self.n2, self.n3 * x - y], x, y)
        return answer
...
class Type20Question:
    ...
    def predict_equation_from_generated_q(self):
        ...
        predicted_n1, predicted_n2, predicted_n3 = assign_all_numbers(prioritized_numbers,
                                                                      self.generated_questions, self.original_question)
...
def assign_all_numbers(prioritized_numbers, generated_questions, original_question):
    found_pairs = []
...
def read_type20_questions():
    return read_all_questions_from_file("../organized_questions/type_20.txt", Type20Question,
                                        first_symbol_to_look_for="*")


if __name__ == "__main__":
    all_type20_questions = read_type20_questions()
    acc_value = test_all_same_type_questions(all_type20_questions, "20")
    print("\n", "~" * 100, "\n====> Accuracy Value for Type 20 Questions =", acc_value * 100, "%")
