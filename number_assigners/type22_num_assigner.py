...
class Type22Question:
    ...
    def predict_equation_from_generated_q(self):
        ...
        """if float(prioritized_numbers[0]) < float(prioritized_numbers[1]):
...
def read_type22_questions():
    return read_all_questions_from_file("type_22.txt", Type22Question,
                                        fix_solution_function=fix_solution_numbers_order)


if __name__ == "__main__":
    all_type22_questions = read_type22_questions()
    acc_value = test_all_same_type_questions(all_type22_questions, "22")
    print("\n", "~" * 100, "\n====> Accuracy Value for Type 22 Questions =", round(acc_value * 100), "%")
