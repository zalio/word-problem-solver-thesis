...
class EquationSet:
    ...
    def solve_equation(self):
        x = Symbol("x")
        y = Symbol("y")
        answer = solve([self.n1 * x + self.n2 * y - self.n3 * self.n4, x + y - self.n5], x, y)
        return answer

    def __str__(self):
        return "n1 = " + str(self.n1) + " ; n2 = " + str(self.n2) + " ; n3 = " + str(self.n3) + \
               " ; n4 = " + str(self.n4) + " ; n5 = " + str(self.n5)


class Type18Question:
    def __init__(self, original_question, num_from_solution):
        ...
        self.solution_equation = EquationSet(num_from_solution[0], num_from_solution[1], num_from_solution[2],
                                             num_from_solution[3], num_from_solution[4])
    ...
    def predict_equation_from_generated_q(self):
        ...
        n1_n2, n3_n4 = find_two_pairs_of_numbers(prioritized_numbers, self.generated_questions, self.original_question)
        if n1_n2 == ["", ""]:
            return None

        n1_n2, n3_n4, common_word_after_n1_n2 = distinguish_n1_n2_and_n3_n4_pairs(n1_n2, n3_n4, self.original_question)
        if common_word_after_n1_n2 == "":
            return None

        predicted_n1, predicted_n2 = assign_numbers_according_to_their_order_in_question(n1_n2, self.original_question)
        predicted_n3 = assign_n3(prioritized_numbers, n1_n2, self.original_question, common_word_after_n1_n2)

        if predicted_n3 == "":
            return None
        predicted_n4, predicted_n5 = assign_remaining_numbers_for_n4_n5_slots(prioritized_numbers, predicted_n1,
                                                                              predicted_n2, predicted_n3)

        if (predicted_n1 == "" or predicted_n2 == "" or predicted_n3 == "" or
                predicted_n4 == "" or predicted_n5 == ""):
            return None
        else:
            try:
...
def find_two_num_around_a_word(prioritized_numbers, split_words, index_of_word):
    found_pair = []
    last_index_of_slice = index_of_word + 10
    if last_index_of_slice > len(split_words):
        last_index_of_slice = len(split_words)
    for i in range(index_of_word + 1, last_index_of_slice):
        split_words[i] = split_words[i].lower()
        if split_words[i] in prioritized_numbers:
            found_pair.append(split_words[i])
            break

    first_index_of_slice = index_of_word - 10
    if first_index_of_slice < -1:
        first_index_of_slice = -1
    for i in range(index_of_word - 1, first_index_of_slice, -1):
        split_words[i] = split_words[i].lower()
        if split_words[i] in prioritized_numbers:
            found_pair.append(split_words[i])
            break
...
def find_two_pairs_of_numbers(prioritized_numbers, generated_questions, original_question):
    ...
    for i in generated_questions:
        if not i.ignore_generated_q:
            ...
            for j in range(len(generated_split)):
                if generated_split[j] == "and" or generated_split[j] == "with" or generated_split[j] == "while":
                    temp_pair = find_two_num_around_a_word(prioritized_numbers, generated_split, j)
        ...
        if not i.ignore_answer:
            ...
            for j in range(len(answer_split)):
                if answer_split[j] == "and" or answer_split[j] == "with" or answer_split[j] == "while":
                    temp_pair = find_two_num_around_a_word(prioritized_numbers, answer_split, j)
                    if temp_pair != ["", ""] and temp_pair not in found_pairs and \
                            reversed(temp_pair) not in found_pairs:
                        found_pairs.append(temp_pair)
    ...
    if len(found_pairs) == 0:
        ...
        for i in range(len(original_split)):
            if original_split[i] == "and" or original_split[i] == "with" or original_split[i] == "while":
                temp_pair = find_two_num_around_a_word(prioritized_numbers, original_split, i)
...
def find_common_word_after_n1_n2(n1_n2, original_question):
    original_split = original_question.replace('. ', ' ').replace('?', '').replace(',', '').replace('-', ' ').split()
    common_word = []
    copy_n1_n2 = n1_n2.copy()
    for i in range(len(original_split)):
        original_split[i] = original_split[i].lower()
        if original_split[i] in copy_n1_n2:
            common_word.append(original_split[i + 1])
            copy_n1_n2.remove(original_split[i])
    if len(common_word) == 2 and common_word[0] == common_word[1]:
...
def assign_n3(prioritized_numbers, n1_n2, original_question, common_word_after_n1_n2):
    copy_prioritized = prioritized_numbers.copy()
    copy_prioritized.remove(n1_n2[0])
    copy_prioritized.remove(n1_n2[1])
    original_split = original_question.replace('. ', ' ').replace('?', '').replace(',', '').replace('-', ' ').split()
    n3 = ""
    for i in range(len(original_split)):
        original_split[i] = original_split[i].lower()
        if original_split[i] in copy_prioritized and original_split[i + 1] == common_word_after_n1_n2:
            n3 = original_split[i]
            break
    return n3


def assign_remaining_numbers_for_n4_n5_slots(prioritized_numbers, predicted_n1, predicted_n2, predicted_n3):
    copy_prioritized = prioritized_numbers.copy()
    ...
    if len(copy_prioritized) == 1:
        return copy_prioritized[0], copy_prioritized[0]
    return "", ""


def distinguish_n1_n2_and_n3_n4_pairs(n1_n2, n3_n4, original_question):
    original_split = original_question.replace('. ', ' ').replace('?', '').replace(',', '').replace('-', ' ').split()
    common_word1_2 = []
    copy_n1_n2 = n1_n2.copy()
    for i in range(len(original_split)):
        original_split[i] = original_split[i].lower()
        if original_split[i] in copy_n1_n2:
            common_word1_2.append(original_split[i + 1])
            copy_n1_n2.remove(original_split[i])

    common_word3_4 = []
    copy_n3_n4 = n3_n4.copy()
    for i in range(len(original_split)):
        original_split[i] = original_split[i].lower()
        if original_split[i] in copy_n3_n4:
            common_word3_4.append(original_split[i + 1])
            copy_n3_n4.remove(original_split[i])

    if len(common_word1_2) == 2 and common_word1_2[0] == common_word1_2[1]:
        return n1_n2, n3_n4, common_word1_2[0]
    elif len(common_word3_4) == 2 and common_word3_4[0] == common_word3_4[1]:
        return n3_n4, n1_n2, common_word3_4[0]
    return n1_n2, n3_n4, ""


def read_type18_questions():
    return read_all_questions_from_file("../organized_questions/type_18.txt", Type18Question,
                                        first_symbol_to_look_for="*")


if __name__ == "__main__":
    all_type18_questions = read_type18_questions()
    acc_value = test_all_same_type_questions(all_type18_questions, "18")
    print("\n", "~" * 100, "\n====> Accuracy Value for Type 18 Questions =", acc_value * 100, "%")
